...
async function main(from, to = from) {
    const files = await (0, glob_1.glob)(from, { withFileTypes: true });
    if (files.length === 0) {
        core.info(`[replacefy] no files found matching ${from}`);
        return false;
    }
    for (const file of files) {
        core.info(`[replacefy] found file ${file.fullpath()}`);
        const contents = await promises_1.default.readFile(file.fullpath(), 'utf8');
        const replaced = contents.replaceAll(/__([A-Z0-9_]+)__|{{([A-Z0-9_]+)}}/g, (_, p1, p2) => {
            const key = p1 || p2;
            return process.env[key] || '';
        });
        await promises_1.default.writeFile(file.fullpath(), replaced);
        core.info(`[replacefy] file ${file.fullpath()} saved.`);
    }
    return true;
}
